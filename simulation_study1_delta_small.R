...
si<- function(beta,delta){
  ...
  sc_value=c(1,exp(seq(log(0.2),log(2.5),length.out = 15)))
  ...
  best_model <- ncvfit(x, y, penalty="SCAD",lambda=best_lambda,penalty.factor=c(rep(sc,p),rep(1,p)),max.iter = 50000)
...
times=100
ans<- matrix(0,10,6)
choose2<- choose1<- matrix(0,10,2*p)
choose3<- matrix(0,10,p)
sc=rep(0,10)
var3<- var2<- var1<- list()
for(j in 1:5){
  k=seq(1,9,by=2)[j]
  delta<- matrix(c(rep(0,25),sample(1:25)),p,1)
  delta<- delta*norm(beta)/norm(delta)*(k/10)
...
save(answer,file='sc_delta_small.RData')
